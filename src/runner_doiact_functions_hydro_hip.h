...
double runner_dopair1_pack_f4_d(struct runner *r, struct scheduler *s,
                                struct pack_vars_pair *restrict pack_vars,
                                struct cell *ci, struct cell *cj,
                                struct task *t,
...
void runner_doself1_launch_f4(
  ...
  //    hipMemcpy(pack_vars->d_task_first_part, pack_vars->task_first_part,
  //               tasks_packed * sizeof(int), hipMemcpyHostToDevice);
  //    hipMemcpy(pack_vars->d_task_last_part, pack_vars->task_last_part,
  //               tasks_packed * sizeof(int), hipMemcpyHostToDevice);
  //    hipMemPrefetchAsync(d_task_first_part_self_dens_f4, tasks_packed *
  //    sizeof(int2), devId, NULL);
  /*Copy cell shifts to device*/
  //    hipMemcpy(pack_vars->d_cellx, pack_vars->cellx,
  //               tasks_packed * sizeof(double), hipMemcpyHostToDevice);
  //    hipMemcpy(pack_vars->d_celly, pack_vars->celly,
  //               tasks_packed * sizeof(double), hipMemcpyHostToDevice);
  //    hipMemcpy(pack_vars->d_cellz, pack_vars->cellz,
  //               tasks_packed * sizeof(double), hipMemcpyHostToDevice);
  ...
  for (int bid = 0; bid < nBundles_temp; bid++) {
    ...
    //	  clock_gettime(CLOCK_REALTIME, &t0hmemcpy);
    //      hipMemPrefetchAsync(&d_task_first_part_self_dens_f4[first_task],
    ...
    ////
    //										//
    // Get error code 	  if (cu_error != hipSuccess) { 		fprintf(
    ...
    // cpuid id is: %i\n ",
    // hipGetErrorString(cu_error), r->cpuid); 		exit(0);
    //	  }
    // #endif
  ...
  *gpu_time +=
  ...
  for (int bid = 0; bid < nBundles_temp; bid++) {
    ...
    hipEventSynchronize(self_end[bid]);
...
void runner_doself1_launch_f4_g(
    ...
    struct part_aos_f4_g_recv *d_parts_recv, hipStream_t *stream, float d_a,
    float d_H, struct engine *e, double *packing_time, double *gpu_time,
    int2 *task_first_part_f4, int2 *d_task_first_part_f4, hipEvent_t *self_end,
    double *unpack_time) {
...
void runner_doself1_launch_f4_f(
    ...
    struct part_aos_f4_f_recv *d_parts_recv, hipStream_t *stream, float d_a,
    float d_H, struct engine *e, double *packing_time, double *gpu_time,
    int2 *task_first_part_f4_f, int2 *d_task_first_part_f4_f,
    hipEvent_t *self_end, double *unpack_time) {
...
void runner_dopair1_launch_f4_one_memcpy(
  ...
  *gpu_time +=
  ...
  for (int bid = 0; bid < nBundles_temp; bid++) {
    ...
    //      hipStreamSynchronize(stream[bid]);
    hipEventSynchronize(pair_end[bid]);
