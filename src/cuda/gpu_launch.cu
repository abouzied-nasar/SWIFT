 ...
 */
__global__ void cuda_launch_pair_density(const struct gpu_part_send_d *restrict d_parts_send,
                                         struct gpu_part_recv_d *restrict d_parts_recv,
                                         const float d_a, const float d_H,
                                         const int bundle_first_part,
  ...
  if (pid < bundle_first_part + bundle_n_parts) {
    ...
    cuda_kernel_pair_density(pi, d_parts_send, d_parts_recv, pid, cj_start, cj_end, d_a, d_H);
  }
}
 ...
 * TODO: Parameter documentation
 */
__global__ void cuda_launch_pair_gradient(const struct gpu_part_send_g *restrict d_parts_send,
  ...
  if (pid < bundle_first_part + bundle_n_parts) {
    ...
    /* Start calculations for particles in cell i*/
    cuda_kernel_pair_gradient(pi, d_parts_send, d_parts_recv, pid, cj_start, cj_end,
                              d_a, d_H);
  }
}
 ...
 * TODO: Parameter documentation
 */
__global__ void cuda_launch_pair_force(const struct gpu_part_send_f *restrict d_parts_send,
  ...
  if (pid < bundle_first_part + bundle_n_parts) {
    ...
    /* Start calculations for particles in cell i */
    cuda_kernel_pair_force(pi, d_parts_send, d_parts_recv, pid, cj_start, cj_end, d_a, d_H);
  }
}
 ...
 * TODO: Parameter documentation
 */
void gpu_launch_pair_density(const struct gpu_part_send_d *restrict d_parts_send,
                             struct gpu_part_recv_d *restrict d_parts_recv,
                             const float d_a, const float d_H,
                             cudaStream_t stream,
 ...
 */
void gpu_launch_pair_gradient(const struct gpu_part_send_g *restrict d_parts_send,
                              struct gpu_part_recv_g *restrict d_parts_recv,
                              const float d_a, const float d_H,
                              cudaStream_t stream,
                              const int num_blocks_x, const int num_blocks_y,
 ...
 * TODO: Parameter documentation
 */
void gpu_launch_self_density(const struct gpu_part_send_d *restrict d_parts_send,
                             struct gpu_part_recv_d *restrict d_parts_recv,
                             const float d_a, const float d_H,
                             cudaStream_t stream,
                             const int num_blocks_x, const int num_blocks_y,
                             const int bundle_first_task,
  ...
  cuda_kernel_self_density<<<gridShape, bsize, shmem_size, stream>>>(d_parts_send, d_parts_recv, d_a, d_H, bundle_first_task, d_task_first_part_f4);
}

/**
 * @brief Launch the self gradient computation on the GPU.
 */
void gpu_launch_self_gradient(const struct gpu_part_send_g *restrict d_parts_send,
                             struct gpu_part_recv_g *restrict d_parts_recv,
                             const float d_a, const float d_H,
                             cudaStream_t stream,
                             const int num_blocks_x, const int num_blocks_y,
  ...
  cuda_kernel_self_gradient<<<gridShape, bsize, shmem_size, stream>>>(d_parts_send, d_parts_recv, d_a, d_H, bundle_first_task, d_task_first_part_f4);
}
