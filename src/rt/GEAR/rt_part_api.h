...
/* const struct rt_part_data* part_get_const_rt_data_p(const struct part* restrict p); */
/* struct spart; */
/* struct rt_spart_data* spart_get_rt_data_p(struct part* restrict p); */
/* const struct rt_spart_data* spart_get_const_rt_data_p(const struct part* restrict p); */
...
part_get_rt_radiation_energy_density(const struct part* restrict p, const size_t g){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_radiation_energy_density(rtd, g);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_radiation_energy_density_p(struct part* restrict p, const size_t g){
...
part_get_const_rt_radiation_energy_density_p(const struct part* restrict p, const size_t g){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_const_radiation_energy_density_p(rtd, g);
}

static __attribute__((always_inline)) INLINE void
part_set_rt_radiation_energy_density(struct part* restrict p, const size_t g, const float energy_density){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float*
part_get_rt_radiation_flux(struct part* restrict p, const size_t group){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
  return rt_part_data_get_radiation_flux(rtd, group);
}

static __attribute__((always_inline)) INLINE const float*
part_get_const_rt_radiation_flux(const struct part* restrict p, const size_t group){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_const_radiation_flux(rtd, group);
}

static __attribute__((always_inline)) INLINE float
part_get_rt_radiation_flux_ind(const struct part* restrict p, const size_t group, const size_t index){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_radiation_flux_ind(rtd, group, index);
}

static __attribute__((always_inline)) INLINE void
part_set_rt_radiation_flux(struct part* restrict p, const size_t group, const float flux[3]){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
  rt_part_data_set_radiation_flux(rtd, group, flux);
}

static __attribute__((always_inline)) INLINE void
part_set_rt_radiation_flux_ind(struct part* restrict p, const size_t group, const size_t index, const float flux){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_energy_flux(const struct part* restrict p, const size_t g){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_energy_flux(rtd, g);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_energy_flux_p(struct part* restrict p, const size_t g){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_energy_flux(struct part* restrict p, const size_t g, const float energy_flux){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float*
part_get_rt_flux_flux(struct part* restrict p, const size_t group){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
  return rt_part_data_get_flux_flux(rtd, group);
}

static __attribute__((always_inline)) INLINE float
part_get_rt_flux_flux_ind(const struct part* restrict p, const size_t group, const size_t index){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_flux_flux(struct part* restrict p, const size_t group, const float flux_flux[3]){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
  rt_part_data_set_flux_flux(rtd, group, flux_flux);
}

static __attribute__((always_inline)) INLINE void
part_set_rt_flux_flux_ind(struct part* restrict p, const size_t group, const size_t index, const float flux_flux){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_flux_dt(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_flux_dt(rtd);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_flux_dt_p(struct part* restrict p){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_flux_dt(struct part* restrict p, const float flux_dt){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_fraction_HI(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_fraction_HI(struct part* restrict p, const float mass_fraction_HI){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_fraction_HII(struct part* restrict p, const float mass_fraction_HII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_fraction_HeI(struct part* restrict p, const float mass_fraction_HeI){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_fraction_HeII(struct part* restrict p, const float mass_fraction_HeII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_fraction_HeIII(struct part* restrict p, const float mass_fraction_HeIII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_number_density_electrons(struct part* restrict p, const float number_density_electrons){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_flux_HI(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_mass_flux_HI(rtd);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_mass_flux_HI_p(struct part* restrict p){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_flux_HI(struct part* restrict p, const float mass_flux_HI){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_flux_HII(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_mass_flux_HII(rtd);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_mass_flux_HII_p(struct part* restrict p){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_flux_HII(struct part* restrict p, const float mass_flux_HII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_flux_HeI(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
  return rt_part_data_get_mass_flux_HeI(rtd);
}

static __attribute__((always_inline)) INLINE float*
part_get_rt_mass_flux_HeI_p(struct part* restrict p){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_flux_HeI(struct part* restrict p, const float mass_flux_HeI){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_flux_HeII(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_flux_HeII(struct part* restrict p, const float mass_flux_HeII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
 ...
 */
static __attribute__((always_inline)) INLINE float
part_get_rt_mass_flux_HeIII(const struct part* restrict p){
  const struct rt_part_data* rtd = part_get_const_rt_data_p(p);
...
static __attribute__((always_inline)) INLINE void
part_set_rt_mass_flux_HeIII(struct part* restrict p, const float mass_flux_HeIII){
  struct rt_part_data* rtd = part_get_rt_data_p(p);
