  ...
  if (!flipped) {
    ...
    for (int pid = 0; pid < scount; pid++) {
      ...
      for (int pjd = 0; pjd < count_j && sort_j[pjd].d < di; pjd++) {
        ...
        /* Compute the pairwise distance. */
        float dx[3] = {(float)(pix - xj[0]), (float)(piy - xj[1]),
                       (float)(piz - xj[2])};
        const float r2 = dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2];
  ...
  /* Sparts are on the right. */
    ...
    for (int pid = 0; pid < scount; pid++) {
      ...
      for (int pjd = count_j - 1; pjd >= 0 && di < sort_j[pjd].d; pjd--) {
        ...
        /* Compute the pairwise distance. */
        float dx[3] = {(float)(pix - pjx[0]), (float)(piy - pjx[1]),
                       (float)(piz - pjx[2])};
        const float r2 = dx[0] * dx[0] + dx[1] * dx[1] + dx[2] * dx[2];
...
void DOSELF1_SUBSET_STARS(struct runner *r, const struct cell *ci,
  ...
  for (int spid = 0; spid < scount; spid++) {
    ...
    for (int pjd = 0; pjd < count_i; pjd++) {
      ...
      /* Hit or miss? */
      if (r2 < hig2) {
        IACT_STARS(r2, dx, hi, part_get_h(pj), spi, pj, a, H);
#if (FUNCTION_TASK_LOOP == TASK_LOOP_DENSITY)
        runner_iact_nonsym_feedback_density(r2, dx, hi, part_get_h(pj), spi, pj,
                                            NULL, cosmo, e->feedback_props,
                                            e->ti_current);
        runner_iact_nonsym_rt_injection_prep(r2, dx, hi, part_get_h(pj), spi,
                                             pj, cosmo, e->rt_props);
#elif (FUNCTION_TASK_LOOP == TASK_LOOP_FEEDBACK)
        error("No subset feedback iact functions do (or should) exist!");
        /* runner_iact_nonsym_feedback_apply(r2, dx, hi, part_get_h(pj), spi,
         * pj, xpj, */
        /*                                   cosmo, e, ti_current); */
#endif
      }
