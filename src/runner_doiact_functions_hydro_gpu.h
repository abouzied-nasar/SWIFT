...
__attribute__((always_inline)) INLINE static void runner_gpu_launch(
  ...
  if (task_subtype == task_subtype_gpu_density){
    ...
    if (cu_error != cudaSuccess) {
      ...
      /*"Once more unto the breach dear friends, once more!"
       *Issue instruction to launch GPU computations*/
      gpu_launch_unique_gradient(buf->d_parts_send_g, buf->d_parts_recv_g, d_a, d_H,
              n_blocks,
              gpu_md->d_cell_i_j_start_end,
              gpu_md->d_block_leaf_id, space_dim, stream[0]);
  ...
  /* Transfer particle data to device */
    ...
    if (cu_error != cudaSuccess) {
       ...
       *
       *Issue instruction to launch GPU computations*/
      gpu_launch_unique_force(buf->d_parts_send_f, buf->d_parts_recv_f, d_a, d_H,
              n_blocks,
              gpu_md->d_cell_i_j_start_end,
              gpu_md->d_block_leaf_id, space_dim, stream[0]);
...
__attribute__((always_inline)) INLINE static void runner_gpu_launch_gradient(
  ...
  if (buf->md.is_pair_task)
    TIMER_TOC2(timer_dopair_gpu_launch_g);
  else
    TIMER_TOC2(timer_doself_gpu_launch_g);
}
...
__attribute__((always_inline)) INLINE static void runner_gpu_launch_force(
  ...
  if (buf->md.is_pair_task)
    TIMER_TOC2(timer_dopair_gpu_launch_f);
  else
    TIMER_TOC2(timer_doself_gpu_launch_f);
}
...
__attribute__((always_inline)) INLINE static void runner_gpu_pack_and_launch(
  ...
  /* Nr of super-level tasks we've accounted for in the meta-data arrays. */
  ...
  while (npacked < md->task_n_leaves) {
    ...
    /* Can we launch? */
    if (md->n_leaves_packed == target_n_leaves)
      md->launch = 1;
