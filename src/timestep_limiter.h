...
timestep_limiter_prepare_force(struct part *restrict p,
  ...
  part_set_timestep_limiter_min_ngb_time_bin(p, num_time_bins + 1);
}
...
__attribute__((always_inline)) INLINE static integertime_t timestep_limit_part(
  ...
  if (part_is_active(p, e)) {
    ...
    part_set_time_bin(p, -part_get_timestep_limiter_wakeup(p) + 2);
    ...
    const timebin_t new_bin = -part_get_timestep_limiter_wakeup(p) + 2;
